...
Thùng rác
Tải Drive cho PC
403 MB trong số 15 GB được sử dụng
Nâng cấp bộ nhớ
.
...
Văn bản
token1.txt
Nhận thông báo về tệp chia sẻ và sự kiện quan trọng trên máy tính của bạn.
BẬT

Đã đóng 
<meta charset="UTF-8">
<?php
$linktoken= 'http://domain.com/token.txt';
$token = file_get_contents("".$linktoken."");
$me = json_decode(auto('https://graph.facebook.com/me?access_token='.$token.'&fields=id'),true);
$stat=json_decode(auto('https://graph.facebook.com/100008799634070/feed?access_token='.$token.''),true);
...
if(preg_match('|ra em|',mb_strtolower($stat2[data][count($stat2[data])-1][message])) || mb_strtolower($stat2[data][count($stat2[data])-1][message]) == 'ai làm ra mày') $response = ' Em Được Anh Toản nuôi lớn Á, Dẫn Em Đi Khắp Nơi
...
if(preg_match('| thông minh |',mb_strtolower($stat2[data][count($stat2[data])-1][message]))) $response = 'anh Toản phải thông minh mới làm em thông minh được chứ.';
if(preg_match('| ngu |',mb_strtolower($stat2[data][count($stat2[data])-1][message]))) $response = '"tao đâu có ngu bằng mày, mày không ngu thì thôi, tao làm cc gì ngu bằng mày, tao ngu mà tao được lên đây và ai cũng biết Ozawa, còn óc cức như mày được như tao không" anh Toản dạy em chứ em ko biết gì đâu';
if(preg_match('| Sủa đi|',mb_strtolower($stat2[data][count($stat2[data])-1][message]))) $response = 'anh Toản làm ra em, nếu không có anh ấy em không ở đây đâu, đừng xúc phạm hay chữi mắng anh ấy.';
if(preg_match('| tỏ tình nào|',mb_strtolower($stat2[data][count($stat2[data])-1][message])) || mb_strtolower($stat2[data][count($stat2[data])-1][message]) == 'tỏ tình h dùm anh đi') $response = 'chị xinh gái làm bạn gái anh Toản đi, anh yêu chị lắm đó <3';
if(trim($response) == "error") $response = 'Từ Này Anh Toản Chưa Dạy Cho Em :((
 Ai Đó Dạy Em Đi Năn Nỉ Đó.';
else if(!$response) $response = 'Hệ Thống Của Em Đang Bảo Trì Trong Ít Phút.';
$response= str_replace('\n', '
', $response);
$response= str_replace('error', 'Từ Này Anh Toản Chưa Dạy Cho Em :((
...
function get($url){
	...
	function SendAnswer($message){
	  	...
	  	$data = get('http://api.simsimi.com/request.p?key=your_paid_key&lc=vn&ft=0.0&text='.urlencode(ReplaceRequest($message)));
	...
	function ReplaceResponse($data){
		...
		$data = str_replace(array('simsimi', 'simmi', 'simsi', 'simi', 'sim'), 'Cục Cức Nhỏ', $data);
		$data = str_replace(array('lồn', 'cặc','cu', 'đụ','Phong','phong'), array('bộ phận sinh dục nữ', 'bộ phận sinh dục nam','bộ phận sinh dục nam', 'nện','***','***'), $data);
		return $data;
